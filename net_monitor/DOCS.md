...
- **Busy-line guard.** Before a scheduled test it reads the router's WAN traffic
  over UPnP; if more than `busy_threshold_mbps` is flowing it waits
  `busy_retry_minutes` and retries, skipping the hour only if the line stays busy.
  Manual tests always run. Scheduled tests can be paused from the dashboard.
...
| Entity | Meaning |
|---|---|
| `binary_sensor.net_monitor_internet` | `on` = international internet reachable (attributes: latency, jitter, loss, online/offline since) |
