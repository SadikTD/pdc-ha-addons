...
export function loadOptions(file = `${DATA}/options.json`) {
  ...
  const options = { token: String(o.api_token || ''), sender: digits(o.sender_number), recipient: digits(o.recipient_number) };
...
export function createLedger(file = `${DATA}/sent.json`) {
  ...
  return {
    get: key => entries[key],
    set(key, value) { entries[key] = { ...value, at: Date.now() }; save(); },
    delete(key) { delete entries[key]; save(); },
  };
}
...
export function createHandler({ options, ledger, wa }) {
  ...
  return async (req, res) => {
    ...
    if (!tokenMatches(req.headers.authorization, options.token)) return reply(res, 401, { status: 'unauthorized' });
    ...
    if (digits(body.to) !== options.recipient) return reply(res, 403, { status: 'recipient_not_allowed' });
